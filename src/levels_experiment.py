...
def main():
    ...
    OPEN_MOUTH_THRESHOLD = 20
    BROW_EYE_DISTANCE_THRESHOLD = 29
    HEAD_TILT_THRESHOLD = 20 #15 # smaller threshold for more sensitivity
    NOSE_POSCHANGE_THRESHOLD = 20 
    ...
    try:
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                # Gesture detection
                mouth_open = distance(get_point(MOUTH_TOP), get_point(MOUTH_BOTTOM)) > OPEN_MOUTH_THRESHOLD

                # Frown detection: brow down relative to eyes
                left_brow_eye = distance(get_point(LEFT_BROW), get_point(LEFT_EYE))
                print("left_brow_eye",left_brow_eye)
                right_brow_eye = distance(get_point(RIGHT_BROW), get_point(RIGHT_EYE))
                print("right_brow_eye",right_brow_eye)
                
                # nose_chin = distance(get_point(NOSE_TIP), get_point(UPPER_CHIN))
                # print("nose_chin",nose_chin)
                
                # frowning = left_brow_eye < 33 and right_brow_eye < 33 and nose_chin < 63 #28
                frowning = left_brow_eye < BROW_EYE_DISTANCE_THRESHOLD and right_brow_eye < BROW_EYE_DISTANCE_THRESHOLD
                ...
                # REVERSE_GEAR 1st Implementation
                # if prev_nose_y is not None:
                #     nodding = abs(nose_y - prev_nose_y) > NOSE_POSCHANGE_THRESHOLD
                # else:
                #     nodding = False
                # prev_nose_y = nose_y
                
                # REVERSE_GEAR 2nd Implementation
                # if prev_nose_y is not None and now - prev_nod_time > 1.0:
                #     delta_y = nose_y - prev_nose_y
                #     if delta_y < -NOSE_POSCHANGE_THRESHOLD:
                #         reverse_mode = True
                #         control.reverse = reverse_mode
                #         # vehicle.apply_control(control)
                #         prev_nod_time = now
                #     elif delta_y > NOSE_POSCHANGE_THRESHOLD:
                #         reverse_mode = False
                #         control.reverse = reverse_mode
                #         # vehicle.apply_control(control)
                #         prev_nod_time = now
                ...
                control.brake = 1.0 if frowning else 0.0
                control.steer = -0.3 if head_tilt_delta > HEAD_TILT_THRESHOLD else (0.3 if head_tilt_delta < -HEAD_TILT_THRESHOLD else 0.0)
